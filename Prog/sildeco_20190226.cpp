...
void funcReadFile(const char* filename_obj, const char* filename_conf, const char* filename_tile)
{
	...
	while(1)
	{
		...
		for(int i=0;i<o->ntile;i++)
		{
			...
			size_t strloc2=strtmp.find(',',strloc1+1);
			x=atoi(strtmp.substr(0,strloc1).c_str());
			y=atoi(strtmp.substr(strloc1+1,strloc2-strloc1-1).c_str());
			percent=atof(strtmp.substr(strloc2+1,strtmp.size()-strloc2-1).c_str());
