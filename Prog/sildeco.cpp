...
int main(int argc, char* argv[])
{
	...
	funcGurobi(0, 0.05, atoi(argv[4]), 0.0, 0.0, 1, 1); // min/max, heu, time, absgap, gap, display, focus (quick feas.)
	funcScreenMessage();
...
void funcP1Prepare()
{
	...
	normal_distribution<double> distribution(0.0,(maxXtile+maxYtile)/32);
	for(set<object*>::iterator setit=seto.begin();setit!=seto.end();setit++)
	{
		object *o=*setit;
		...
		for(int i=0;i<nrolls;i++)
		{
			...
			if(exresult>=0&&exresult<pow(pow(maxXtile,2)+pow(maxYtile,2),0.5))
				ploc[int(exresult)]++;
		}
		for(auto tl:o->settileloc)
		for(int i=0;i<maxXtile;i++)
		for(int j=0;j<maxYtile;j++)
			o->dttile[i][j]+=ploc[(int)pow(pow(abs(get<0>(tl)-i),2)+pow(abs(get<1>(tl)-j),2),0.5)]*get<2>(tl);
		for(int i=0;i<maxXtile;i++)
		for(int j=0;j<maxYtile;j++)
...
void funcP2Prepare()
{
	...
	// record current drying time for each tile
	recdttile=new double **[maxL->intrval+1];
	for(int i=1;i<=maxL->intrval;i++)
		recdttile[i]=new double *[maxXtile];
	for(int i=1;i<=maxL->intrval;i++)
	for(int j=0;j<maxXtile;j++)
		recdttile[i][j]=new double [maxYtile];
	for(int i=1;i<=maxL->intrval;i++)
	for(int j=0;j<maxXtile;j++)
	...
	for(int j=0;j<maxXtile;j++)
	for(int k=0;k<maxYtile;k++)
		recdttile[o->vl->intrval][j][k]+=o->dttile[j][k];
}
...
void funcScreenMessage()
{
	...
	for(int i=1;i<=maxL->intrval;i++)
	{
		...
		if (vecl.size()!=0) // P2
		{
			//cout<<"P2forGurobi"<<endl;
			cout<<"("<<vecl[i-1]->vmaxdt->dourval<<")";
			totdt+=vecl[i-1]->vmaxdt->dourval;
		}
		cout<<": ";
		for(set<object*>::iterator setit=seto.begin();setit!=seto.end();setit++)
	...
	cout<<"Total required drying time: "<<totdt<<endl;
}
void funcWriteFile(const char* filename)
{
	for(int i=1;i<=maxL->intrval;i++)
	{
		...
		fstream WriteFile_heat_nonopt;
		strtmp=filename;
		strtmp+="_heat_nonopt";
		strtmp+=convi.str();
		strtmp+=".txt";
		WriteFile_heat_nonopt.open(strtmp.c_str(), fstream::out);
		for(int k=0;k<maxYtile;k++)
		for(int j=0;j<maxXtile;j++)
			WriteFile_heat_nonopt<<j+1<<" "<<k+1<<" "<<recdttile[i][j][k]<<endl;
		WriteFile_heat_nonopt.close();
	}
...
void funcReadFile(const char* filename_obj, const char* filename_conf, const char* filename_tile)
{
	...
	while(1)
	{
		...
		{
			ReadFile_tile>>strtmp; // tile coordinate and occupation percentage
			strtmp=strtmp.substr(1,strtmp.size()-2);
			int x, y;
			double percent;
			size_t strloc1=strtmp.find(',');
			x=atoi(strtmp.substr(0,strloc1).c_str());
			y=atoi(strtmp.substr(strloc1+1,strtmp.size()-strloc1-1).c_str());
			ReadFile_tile>>strtmp;
			percent=atof(strtmp.substr(0,strtmp.size()-1).c_str());
			o->settileloc.insert(make_tuple(x-1,y-1,percent));
		}
	}
	ReadFile_tile.close();
}
