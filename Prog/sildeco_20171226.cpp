...
void funcObjConf()
{
	// contacted big obj and small obj, printing order specification
	// very close 2 objs, denote them as conflict
}
void funcPathtoObj()
{
	for(set<path*>::iterator setit=setpath.begin();setit!=setpath.end();setit++)
	{
		object* o=new object;
		seto.insert(o);
		o->p=*setit;
		// pt, dt needs to be re-designe needs to be re-designedd
		o->pt=areatopt;
		o->dt=areatodt;
	}
}
...
void funcWriteFile(const char* filename)
{
	...
	{
		WriteFile<<-2<<endl;
		WriteFile<<(*setit)->vx.size()<<endl;
		for(int i=0;i<(*setit)->vx.size();i++)
		{
			WriteFile<<(*setit)->vx[i]<<endl;
			WriteFile<<(*setit)->vy[i]<<endl;
		}
	}
	WriteFile.close();
/*	fstream WriteFile;
...
void funcReadFile(const char* filename)
{
	...
	while(1)
	{
		...
		if(ReadFile.eof())
			break;
		if(strtmp.compare("stream")==0) // drawing start
		{
			while(1)
			{
				getline(ReadFile,strtmp);
				if(strtmp.compare("endstream")==0) // drawing end
					break;
				string strfind=" ", strex="", str1, str2;
				std::size_t found=strtmp.find(strfind);		
				if(found!=std::string::npos)		
				{
					str1=strtmp.substr(0,found);
					found=strtmp.find(strfind,found+1);
				}
				if(found!=std::string::npos)		
				{
					str2=strtmp.substr(str1.size()+1,found-str1.size()-1);
					strex=strtmp.substr(found+1,1);
				}
				if(strex.compare("m")==0) // path start
				{
					path *p=new path;
					double x=atof(str1.c_str());
					double y=atof(str2.c_str());
					p->vx.push_back(x);
					p->vy.push_back(y);
					found+=2; // pass m
					while(1)
					{
						std::size_t newfound=strtmp.find(strfind,found+1);
						if(newfound!=std::string::npos)		
						{
							str1=strtmp.substr(found+1,newfound-found-1);
							found=strtmp.find(strfind,found+1);
cout<<str1<<" ";
						}
						else
							break;
						found=newfound;
						newfound=strtmp.find(strfind,found+1);
						if(newfound!=std::string::npos)		
						{
							str2=strtmp.substr(found+1,newfound-found-1);
							found=strtmp.find(strfind,found+1);
cout<<str2<<" ";
						}
						else
						...
						if(newfound!=std::string::npos)		
						{
							strex=strtmp.substr(found+1,newfound-found-1);
							found=strtmp.find(strfind,found+1);
cout<<strex<<endl;
						}
						else
							break;
						if(strex.compare("m")!=0&&strex.compare("l")!=0)
							break;
						x=atof(str1.c_str());
						y=atof(str2.c_str());
						p->vx.push_back(x);
		...
		else if(strtmp.compare("  /MediaBox [")==0) // boundary
		{
			ReadFile>>strtmp;
			ReadFile>>strtmp;
			ReadFile>>maxX;	
